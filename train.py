        ...
        for epoch in range(1, (1 + self.first_stage_epochs + self.second_stage_epochs)):
            ...
            for train_data in pbar:
                if net_type == 'tiny':
                    ...
                    #print(train_data[4].shape, train_data[5].shape, train_data[6].shape)
                    _, summary, train_step_loss, global_step_val = self.sess.run(
                        [train_op, self.write_op, self.loss, self.global_step], 
                        feed_dict={self.input_data: train_data[0],
                                   self.label_sbbox: train_data[3], self.label_mbbox: train_data[2], self.label_lbbox: train_data[1],
            ...
            for test_data in self.testset:
                if net_type == 'tiny':
                    ...
                    test_step_loss = self.sess.run(self.loss, 
                        feed_dict={self.input_data: test_data[0], 
                                   self.label_sbbox: test_data[3], self.label_mbbox: test_data[2], self.label_lbbox: test_data[1], 
